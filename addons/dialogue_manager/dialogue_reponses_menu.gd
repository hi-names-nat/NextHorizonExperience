	...
	set(value):
		...
		if responses.size() > 0:
			for response in responses:
				...
				if "response" in item:
					item.response = str(get_child_count()) + ": " + response
				# Otherwise assume we can just set the text
				else:
					item.text = str(get_child_count()) + ": " + response.text
...
func response_ray(response_num: int) -> bool:
	if response_num > responses.size(): return false
	var item = responses[response_num-1]
	print(item)
	if !item.is_allowed: return false
	response_selected.emit(item)
	return true
